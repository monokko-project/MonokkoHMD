...
public class YOLOv8 : MonoBehaviour {
    ...
    public IEnumerator Execute(Texture2D tex)
    {
        isExecuting = true;
        this.detects = null;
        _image = tex;
        ...
        // 推論実行
        //var asyncJob = worker.StartManualSchedule(inputTensor);
        //await asyncJob;

        int stepsPerFrame = 9;
        var enumerator = worker.StartManualSchedule(inputTensor);
        int step = 0;
        ...
        */
        ...
        // UI描画する場合
        //Destroy(_imageView.texture);
        //_imageView.texture = image;
        
        Destroy(texture);

        // 結果を返す
        //検出結果は左上が原点だが、Texture2Dやスクリーン座標は左下が原点なので上下を入れ替える
        List<Detect> detects = new List<Detect>();
        float toScreenPointX = (Screen.width / (float)_image.width) * scaleX;
        float toScreenPointY = (Screen.height / (float)_image.height) * scaleY;

        foreach (DetectionResult ditect in ditects) {
            detects.Add(new Detect {
                classId = _labels[ditect.classId],
                score = ditect.score,
                x1 = ditect.x1 * toScreenPointX,
                x2 = ditect.x2 * toScreenPointX,
                y1 = (_image.height - ditect.y1) * toScreenPointY,
                y2 = (_image.height - ditect.y2) * toScreenPointY
            });
        }
        //return detects;
